...
const BottomNavBar = () => {
    ...
    const navItems = [
        ...
        { path: '/minha-conta', icon: <UserCircle size={28} />, activeIcon: <UserCircle size={28} weight="fill" />, label: 'Perfil' },
    ];
        ...
        <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[95%] max-w-md md:hidden z-30" ref={menuRef}>
            <div className="relative flex h-16 items-center justify-around rounded-full border-2 border-gray-200/10 bg-[var(--bg-color)]/60 backdrop-blur-md shadow-lg">
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button 
                                ...
                                className={`flex flex-col items-center justify-center w-full h-full transition-colors duration-300 ${isActive ? 'text-[var(--primary-color)]' : 'text-gray-400 hover:text-white'}`}
                            >
                ...
                <button 
                    ...
                    className={`absolute left-1/2 -translate-x-1/2 -translate-y-[28px] h-16 w-16 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 ${location.pathname === chatbotItem.path ? 'bg-[var(--primary-color-hover)] text-gray-300' : 'bg-[var(--primary-color)] text-black'}`}
                >
                    {location.pathname === chatbotItem.path ? chatbotItem.activeIcon : chatbotItem.icon}
                </button>
            </div>
        </nav>
    );
